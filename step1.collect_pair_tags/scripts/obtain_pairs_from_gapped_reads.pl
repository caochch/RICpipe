#!/usr/bin/perl
die "Collect spliced reads with gaps from mapping results of read1 and read2\n\nUsage: five parameters are required\nperl $0 <read_1Aligned.out.sam> <read_2Aligned.out.sam> <read_1Chimeric.out.sam> <read_2Chimeric.out.sam> <outFile.sam>"  if(@ARGV != 5);
